...
# The only top-level keys .claude/settings.json may carry, and the only
# sub-keys allowed inside "permissions".
#
# An allowlist of *keys*, not just of values, because the dangerous settings are
# ones this file never mentions. `"defaultMode": "bypassPermissions"` disables
# the approval prompt for every tool call - strictly worse than any single entry
# the value allowlist polices - and a guard that only reads "allow" and "hooks"
# waves it straight through. Anything new must be reviewed before it ships.
ALLOWED_SETTINGS_KEYS = {"permissions", "hooks"}
ALLOWED_PERMISSION_KEYS = {"allow"}

# Files that must never be committed: they configure tool access or auto-approval
# outside the reviewed settings.json, so a fork would inherit them silently.
...
REQUIRED_IGNORE_RULES = [
    # The user's populated profile. The tracked files under
    # .claude/skills/job-application-assistant/ are blank templates; the real
    # answers are written to profile/ so a fork cannot commit them.
    "profile/*",
    # Search settings: job titles, target locations, salary floor. Reveals what
    # the user is looking for, which is exactly what a public fork must not.
...
def check_permissions() -> None:
    ...
    for key in sorted(set(data) - ALLOWED_SETTINGS_KEYS):
        errors.append(
            f".claude/settings.json: unreviewed top-level key {key!r}. Settings "
            "outside the reviewed set can grant tool access or suppress prompts "
            "without appearing in the permissions allowlist. If it is "
            "intentional, add it to ALLOWED_SETTINGS_KEYS in "
    ...
    for key in sorted(set(permissions) - ALLOWED_PERMISSION_KEYS):
        errors.append(
            f".claude/settings.json: unreviewed permissions key {key!r}. "
            "Keys such as 'defaultMode' can disable the approval prompt for "
            "every tool call, which no entry-level allowlist can see. If it is "
            "intentional, add it to ALLOWED_PERMISSION_KEYS in "
            "tools/security_guards.py in the same PR."
        )
...
def check_forbidden_config_files() -> None:
    """Fail if a tool-access config outside the reviewed settings.json is tracked.

    `.mcp.json` and `.claude/settings.local.json` are read automatically by agent
    runtimes. Committing either hands every fork a configuration nobody reviewed,
    which is the same failure the settings.json allowlist exists to prevent.
    """
    for rel in FORBIDDEN_CONFIG_FILES:
        if (ROOT / rel).exists():
            errors.append(
                f"{rel}: must not be committed. It configures tool access "
                "outside the reviewed .claude/settings.json, so a fork would "
                "inherit it silently. Add it to .gitignore and remove it from "
                "the working tree."
            )
