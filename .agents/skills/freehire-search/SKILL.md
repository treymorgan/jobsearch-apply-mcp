...
> It queries freehire's public JSON API, so results are structured (skills,
> seniority, region facets) rather than parsed from markup.
...
**public and unauthenticated**, so there is no signup or API key.
...
> **Location is a facet, not free text.** Rather than a free-text location
> string, freehire filters geography through structured `--region`/`--country`/`--city`
