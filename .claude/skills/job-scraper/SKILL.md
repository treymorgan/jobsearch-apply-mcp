...
description: >
  ...
  (job-board APIs and any skills added with /add-portal). Deduplicates
...
3. Read `jobsearch.config.json` for the configured search market, query list, filters, and deal-breakers
4. Read `search-queries.md` (this directory) for the search strategy guide
...
2. Translate the query terms from `jobsearch.config.json` into that portal's flag format (e.g. `--key`, `--search-string`, `--query`, filter codes - whatever the portal's SKILL.md specifies).
3. Scope to the last 14 days using the portal's supported recency **filter** flag (`--jobage`, `--since <YYYY-MM-DD>`, etc. - as documented per portal). A portal with **no recency flag** still gets scoped: every portal's search output carries a `date` field, so filter client-side - drop results whose `date` is older than 14 days after the call returns, and never invent a flag the portal's SKILL.md does not document (the CLIs reject unknown flags). A sort flag is a sort, and a sort is not a filter - pairing one with a `--limit` is a defensible approximation on a portal that offers nothing better, but apply the client-side date filter on top all the same.
...
```json
{
  "seen": {
    "<url_or_company_title_key>": {
      ...
      "portal": "<source portal skill, e.g. freehire-search>",
...
3. **Configured market scope, verified at the detail layer.** This workflow targets the market defined in `jobsearch.config.json`: remote roles in `search.remote_location` using the configured `search.remote_filter` and `search.remote_region`, plus any onsite or hybrid role inside `search.local_metro` when that optional list is set. The search lanes are `remote` and `local`; the remote-status classifier returns `local` for jobs accepted through the configured commutable-area lane. Portal remote filters are unreliable and must never be trusted on their own: a portal remote filter can still return city-based rows, and search output may carry **no workplace-type field at all**, only a `location` string that names a city even for fully remote roles. So a search-result `location` is *evidence of nothing* about remote status. Confirm remote from the **detail description text** before presenting a job as remote (look for "remote", "work from anywhere", "distributed team", and for the disqualifiers "hybrid", "X days in office", "onsite"). A posting whose detail text says hybrid or onsite **outside the configured `search.local_metro`** is out of scope even when a remote filter returned it. A remote posting restricted outside the configured remote market, or one requiring work authorization the candidate lacks, is likewise out of scope. When the detail fetch fails and remote status can't be confirmed, present the job with an explicit "remote unconfirmed" note rather than assuming either way.
