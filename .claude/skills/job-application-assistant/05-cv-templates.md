...
**Compile with:** **tectonic** - the only TeX toolchain installed on this machine. It auto-downloads packages on first use and handles moderncv + fontawesome5 cleanly (verified). `lualatex`, `xelatex` and `pdflatex` are not installed.
...
Write 5-7 lines that function as an "elevator pitch": a concise, compelling introduction explaining why you're qualified for *this specific role*. Emphasize what the employer gains from hiring you.
...
A profile statement should be rewritten for each target role using only facts from the candidate profile. Start with the candidate's role level and strongest fit, then connect two or three relevant strengths to the employer's needs. Prefer concrete scope, tools, customers, outcomes or operating context over broad adjectives.

**Generic illustrative example for a coordination role:**
> Mid-career professional with experience coordinating projects, improving team routines and keeping stakeholders aligned through clear documentation and follow-up. Combines practical operations support with calm communication, organized tracking and a service-minded approach to internal and external customers.

**Generic illustrative example for a people leadership role:**
> Team leader with experience guiding small cross-functional groups through planning, delivery and continuous improvement. Known for clarifying priorities, building repeatable processes and helping colleagues turn ambiguous requests into practical next steps.
...
- **`%` fails silently.** An unescaped `%` starts a LaTeX comment: the compile succeeds with zero errors, and everything after the `%` on that line vanishes from the PDF. `Cut inference latency by 40% and saved $2M annually` renders as "Cut inference latency by 40" - the bullet keeps its impressive-looking fragment and loses the actual result. Quantified achievement bullets are exactly where the guidance steers you ("use numbers where possible"), so check every `%` in every bullet before compiling.
...
1. Run `tectonic -X compile main_<company>_<role>.tex`
...
- **No garbled output.** `(cid:NNN)` markers or `�` characters mean a font is embedded without a Unicode mapping - an ATS sees the same garbage. This shows up with unusual fonts in custom templates, not with the stock moderncv setup under tectonic.
