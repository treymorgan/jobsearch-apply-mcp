...
3. **Compile command** - the full command `/apply` and Step 4's test compile will run, using `<file>` (no extension) as the placeholder for the output basename:
   - **`.tex` source**: this machine has **tectonic** only - no TeX Live or MiKTeX, so `lualatex`, `xelatex` and `pdflatex` are unavailable. Render as `tectonic -X compile <file>.tex`. Tectonic's XeTeX-based engine covers `fontspec` and path-loaded fonts, so a template needing those still works; if the user insists on a different engine, tell them it must be installed first.
...
- **Engine/toolchain:** tectonic | typst | <other> (display label only)
- **Page limit:** <N> page(s)
...
## Compile command
    ...
    cd <output dir> && <the full declared command, e.g. tectonic -X compile <file>.tex or typst compile <file>.typ <file>.pdf>
