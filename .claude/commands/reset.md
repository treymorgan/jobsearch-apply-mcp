...
Use Glob to list all files present in `documents/cv/`, `documents/linkedin/`, `documents/diplomas/`, `documents/references/`, `documents/postings/`, `documents/interview/`, and `documents/applications/`. Present as:
...
```bash
rm -rf profile/*
touch profile/.gitkeep
rm -f jobsearch.config.json
rm -f documents/cv/*
