...
Otherwise, start with a short orientation about where the answers will be written,
**before anything is written, not after**.

Your personal answers are written to two locations that are **gitignored**, so they are
never committed even if this working copy is a public fork:

- `profile/` at the workspace root: your candidate profile, behavioral profile, job
  evaluation preferences, interview examples and `CLAUDE.md` overview.
- `jobsearch.config.json` at the workspace root: your search terms, target locations
  and salary floor.

The files under `.claude/skills/job-application-assistant/` are **blank templates that
stay tracked**. Read them for structure; never write the user's real details into them.
The same applies to `cv/main_example.tex` and `cover_letters/cover_example.tex`, which
are shipped samples: write the user's real CV to `cv/main_<name>.tex`, which is
gitignored.

Do a quick sanity check that this protection is actually in place. Run
`git check-ignore -q profile/ jobsearch.config.json`; if it exits non-zero, the ignore
rules are missing or have been edited, so warn and wait:

> **Heads-up before we start:** `profile/` and `jobsearch.config.json` are not being
> ignored by git in this working copy, so anything I write there could be committed and,
> if you push to a public remote, published. Restore the ignore rules (`python3
> tools/security_guards.py` will tell you which are missing) before continuing. Want to
> continue anyway?

If the check passes, continue silently. There is no need to ask about remote visibility:
...
- **Job portals:** "The framework ships `freehire-search`, enabled by default. `/scrape` auto-discovers whatever portal skills are installed under `.agents/skills/` and skips any with `enabled: false`. Which boards do you want covered?" If the user needs a board that is not shipped, guide them to `/add-portal`. WebSearch/`site:` queries remain the fallback for portals without a CLI skill.
...
Once data collection is complete, populate the files below.

**Write destination, non-negotiable:** every file in this step is written to `profile/`
at the workspace root, **not** to `.claude/skills/job-application-assistant/` and not to
the repository root. The skill directory holds blank templates that must stay clean so
this project can be shared publicly; `profile/` is gitignored so the user's real details
cannot be committed. Create `profile/` if it does not exist, and copy the corresponding
template out of the skill directory as your starting structure.

**For Path A**, the seven files are already populated by Step A7; check each before
writing and skip if its content is no longer placeholder text.

### 1. Write `profile/CLAUDE.md`
Start from the tracked `CLAUDE.md` template. Replace all `[PLACEHOLDER]` tokens with the
user's actual information, and delete the quoted "this file is a template" block at the
top. Keep the structure, workflow and verification checklist intact.
...
Replace the skill match areas with the user's actual skills:
...
### 5. Update `profile/05-cv-templates.md` *(Path B and C; skip if Path A populated it)*
Add role-specific profile statement templates based on their background.

### 6. Update `profile/07-interview-prep.md` *(Path B and C; skip if Path A populated it)*
...
### 6b. Write `jobsearch.config.json`
Copy `jobsearch.config.example.json` and fill in the user's search terms, target
locations, optional commutable area, title filters and salary floor from what they told
you. This is what the MCP server reads, so a search will return nothing useful until it
exists. See `docs/CONFIGURATION.md` for every field.

### 7. Write the user's master CV
Write to `cv/main_<lastname>.tex`, which is gitignored. Start from
`cv/main_example.tex` for structure, but **never edit that file**: it is the shipped
sample. Replace the sample personal data with the user's actual name and contact
details, and add their education and experience entries.

### 8. Write `profile/search-queries.md`
Copy `.claude/skills/job-scraper/search-queries.md` as your starting structure, then
replace all placeholder tokens with the user's actual information from Section 9 (or the equivalent follow-up questions in Path A's Step A7):
...
- Keep these consistent with what you wrote to `jobsearch.config.json` in step 6b; the
  config file is what the MCP server actually reads, this file is the human-readable
  strategy behind it
...
> - `profile/05-cv-templates.md` - CV templates with your profile statements
> - `profile/07-interview-prep.md` - STAR examples from your experience
> - `profile/CLAUDE.md` - Your workspace overview
> - `profile/search-queries.md` - Job search strategy for `/scrape`
> - `jobsearch.config.json` - Search terms and hard limits, read by the MCP server
> - `cv/main_<yourname>.tex` - Your LaTeX CV
>
> **Privacy note:** every file above holds your personal data, and every one of them is
> **gitignored**. Nothing you just told me is tracked by git, so this working copy stays
> safe to fork or publish. Verify any time with `python3 tools/security_guards.py`.
>
> **Try it out:**
...
If Path A left any STAR stubs in `profile/07-interview-prep.md`, also note:

> Path A flagged [N] STAR candidate stubs in `profile/07-interview-prep.md` that need your situation/task/action/result details before you use them in interviews.
