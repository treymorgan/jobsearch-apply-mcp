...
**Resolve the active template (do this once, reuse everywhere below):** if `05-cv-templates.md` or `06-cover-letter-templates.md` opens with an `ACTIVE-TEMPLATE` managed block (inserted by `/add-template`), read its declared **source extension** and **compile command** — these override the stock `.tex`/tectonic (CV) and `.tex`/tectonic (cover letter) defaults for the rest of this workflow. Call these `<CV_EXT>`/`<CV_COMPILE>` and `<COVER_EXT>`/`<COVER_COMPILE>`; where no block is present, both default to `.tex` and the stock tectonic command. Every `.tex` reference below is really `<CV_EXT>` or `<COVER_EXT>` — stock behavior is unchanged, this only matters when a custom template is active.
...
- **Match the language of the job posting** (an English posting gets an English cover letter; a posting written in another language gets a cover letter in that language)
...
- **Both documents** compile with **tectonic** (`tectonic -X compile <file>.tex`). Tectonic is the only TeX toolchain installed on this machine — there is no TeX Live or MiKTeX, so `lualatex`/`xelatex`/`pdflatex` do not exist. Tectonic uses a XeTeX-based engine, auto-fetches missing packages on first use, and handles both moderncv+fontawesome5 and cover.cls+fontspec cleanly (both verified).
- Tectonic writes the PDF and cleans up its own intermediates, so Step 5e usually has nothing to delete.
- **Custom template active:** run its declared `<CV_COMPILE>`/`<COVER_COMPILE>` command instead, substituting the actual filename for `<file>`. Never fall back to tectonic when a custom template's compile command is a different toolchain (e.g. `typst compile`) — that command is what the manifest actually verified in `/add-template` Step 4.
...
### 5b-bis. Mechanical style check (both documents)

House style rule 1 in `03-writing-style.md` bans em-dashes. A prompt rule is an
instruction, not a guarantee, and an em-dash that reaches an employer cannot be
recalled - so verify it mechanically against the rendered text layer:

```bash
python3 tools/verify_pdf.py cv/main_<company>_<role>.pdf --no-em-dash
python3 tools/verify_pdf.py cover_letters/cover_<company>_<role>.pdf --no-em-dash
```

The check reports the surrounding sentence on failure. Fix the source (comma,
period, or restructure - never swap in an en-dash to dodge the check) and
recompile. En-dashes are deliberately allowed: moderncv renders legitimate date
ranges with them.

Requires `pdftotext`. If poppler is missing the command says so; fall back to
reading the PDF and searching for the character yourself, and note the degraded
mode in the Step 6 report.
