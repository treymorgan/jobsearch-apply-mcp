...
3. **Coverage** - which US regions or job categories the portal covers. This drives the trigger phrases in `SKILL.md`.
...
4. **Check access requirements and terms.**
   ...
   - If robots.txt disallows the paths or the portal's terms prohibit automated access, tell the user plainly and let them decide whether to proceed for personal use. If they proceed, the generated `SKILL.md` **must** carry a prominent personal-use-only warning (keep volume low, no commercial or bulk use, own responsibility). **Prefer a portal with an official API** where one exists - it is more reliable than parsing markup, and it avoids the question entirely. A free API key is worth the extra setup step.
...
**Canonical reference:** read `.agents/skills/freehire-search/` before generating - it is the worked example of this exact structure. Copy its architecture, not its portal-specific parsing.
...
- **HTML parsing** (only when the portal has no API): split the response into per-result chunks and parse each independently, so one malformed card cannot break the rest. A JSON API needs none of this, which is one more reason to prefer one.
- **Dependencies:** default to **zero runtime dependencies** (plain `bun` + `fetch` + regex parsing) like `freehire-search` - `bun install` should only pull dev types. Only add a parsing library if the portal's markup genuinely defeats chunked regex parsing, and say so in the README.
...
- Zero runtime dependencies by default, matching `freehire-search` - a portal skill should run on a fresh clone with nothing but `bun`.
